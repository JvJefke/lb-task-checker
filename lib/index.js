...
class LbTaskChecker {
	 ...
	 */
	registerTask(task) {
		return this.model.findOneAndUpdate({ key: task.key }, task, { upsert: true, setDefaultsOnInsert: true }).exec();
	}
	...
	reserve(key, expires, instance) {
		return this.model.update({ key, expires: { $lte: new Date() } }, { $set: { expires, instance } })
